...
import mesh.MeshBuilder.BadMeshException;
import mesh.Particle;
import mesh.ParticleSystem;

import com.jogamp.opengl.util.*;

import effectors.Cube;
import effectors.GravityForce;
...
public class ParticleSystemBuilder implements GLEventListener
{
	...
	ParticleSystemBuilder() throws FileNotFoundException, BadMeshException
	{
		...
		if (!file.exists()) {
			System.err.println("Error: Tried to load a mesh from a non-existant file.");
			return;
		}
		
		meshy = MeshBuilder.buildMesh(file, PS);
		
		// add forces to PS
		PS.F.add(new GravityForce(PS));
		
		// add objects to PS
		PS.O.add(new Cube(PS));
		
		// add constraints
//		PS.C.add(new IncompressibilityConstraint(PS));
//		
//		//add post modifiers
//		PS.M.add(new XSPHViscosity(PS));
//		PS.M.add(new VorticityConfinement(PS));
	...
	class BuilderGUI implements MouseListener, MouseMotionListener, KeyListener
	{
		...
		/** Print writer to print particle positions to file. */
		PrintWriter writer;
		int count = 0;
		...
		//	PS.display(gl); // HIDDEN BY DAN SO WE DON"T HAVE BLUE VERTICES
			
			//Draw meshes ADDED BY DAN
			meshy.display(gl);
		...
		{ 
			...
			try{ 
				...
				if (image) {
					...
					for (Particle p : PS.P) {
						output.write(""+p.getX().x+" "+p.getX().y+" "+p.getX().z+"\n");
					}
					output.close();
				}
...
//			File file = new File("C:/Users/Daniel/workspace/A1_Fluids_v2_2/A1_Fluids_v2/simulation.txt");//C:/Users/Me/Desktop/directory/file.txt");
//			file.getParentFile().mkdirs();
//			outputStream = new PrintWriter(file);//new FileWriter("simulation.txt"));
